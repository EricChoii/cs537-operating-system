...
main(void)
{
  ...
  if(fork() > 0)
  15:	e8 06 04 00 00       	call   420 <fork>
  1a:	85 c0                	test   %eax,%eax
  1c:	7e 0d                	jle    2b <main+0x2b>
    sleep(5);  // Let child exit before parent.
  1e:	83 ec 0c             	sub    $0xc,%esp
  21:	6a 05                	push   $0x5
  23:	e8 90 04 00 00       	call   4b8 <sleep>
  28:	83 c4 10             	add    $0x10,%esp
  exit();
  2b:	e8 f8 03 00 00       	call   428 <exit>
 ...
 159:	31 f6                	xor    %esi,%esi
{
 ...
 178:	50                   	push   %eax
 179:	6a 00                	push   $0x0
 17b:	e8 c0 02 00 00       	call   440 <read>
    if(cc < 1)
...
stat(const char *n, struct stat *st)
{
 ...
 1e1:	e8 82 02 00 00       	call   468 <open>
  if(fd < 0)
 ...
 1f5:	50                   	push   %eax
 1f6:	e8 85 02 00 00       	call   480 <fstat>
  close(fd);
 1fb:	89 1c 24             	mov    %ebx,(%esp)
  r = fstat(fd, st);
 1fe:	89 c6                	mov    %eax,%esi
  close(fd);
 200:	e8 4b 02 00 00       	call   450 <close>
  return r;
...
int thread_create(void (*start_routine)(void *, void *), void* arg1, void* arg2)
{
 ...
 2a0:	e8 4b 05 00 00       	call   7f0 <malloc>
  void* stack;
  if(freeptr == 0)
 2a5:	83 c4 10             	add    $0x10,%esp
 2a8:	85 c0                	test   %eax,%eax
 2aa:	74 6e                	je     31a <thread_create+0x8a>
    return -1;
  if((uint)freeptr % PGSIZE == 0)
 2ac:	89 c2                	mov    %eax,%edx
 2ae:	89 c3                	mov    %eax,%ebx
 2b0:	81 e2 ff 0f 00 00    	and    $0xfff,%edx
 2b6:	75 58                	jne    310 <thread_create+0x80>
    stack = freeptr;
  else
    stack = freeptr + (PGSIZE - ((uint)freeptr % PGSIZE));
  for(int i = 0; i < MAX_PROC; i++){
 2b8:	b9 80 0c 00 00       	mov    $0xc80,%ecx
 2bd:	31 d2                	xor    %edx,%edx
 2bf:	eb 12                	jmp    2d3 <thread_create+0x43>
 2c1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
 2c8:	83 c2 01             	add    $0x1,%edx
 2cb:	83 c1 0c             	add    $0xc,%ecx
 2ce:	83 fa 40             	cmp    $0x40,%edx
 2d1:	74 21                	je     2f4 <thread_create+0x64>
    if(ptrs[i].busy == 0){
 2d3:	83 39 00             	cmpl   $0x0,(%ecx)
 2d6:	75 f0                	jne    2c8 <thread_create+0x38>
      ptrs[i].ptr = freeptr;
 2d8:	8d 14 52             	lea    (%edx,%edx,2),%edx
 2db:	c1 e2 02             	shl    $0x2,%edx
 2de:	89 82 84 0c 00 00    	mov    %eax,0xc84(%edx)
      ptrs[i].stack = stack;
 2e4:	89 9a 88 0c 00 00    	mov    %ebx,0xc88(%edx)
      ptrs[i].busy = 1;
 2ea:	c7 82 80 0c 00 00 01 	movl   $0x1,0xc80(%edx)
 2f1:	00 00 00 
      break;
    }
  }
 ...
 2fe:	e8 c5 01 00 00       	call   4c8 <clone>
 ...
 30f:	90                   	nop
    stack = freeptr + (PGSIZE - ((uint)freeptr % PGSIZE));
 310:	29 d3                	sub    %edx,%ebx
 312:	8d 9b 00 10 00 00    	lea    0x1000(%ebx),%ebx
 318:	eb 9e                	jmp    2b8 <thread_create+0x28>
    return -1;
 31a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
 31f:	eb e5                	jmp    306 <thread_create+0x76>
...
int thread_join()
{
 ...
 338:	56                   	push   %esi
  void* stack;
  int ret = join(&stack);
 339:	8d 45 e4             	lea    -0x1c(%ebp),%eax
{
 33c:	53                   	push   %ebx
 33d:	83 ec 28             	sub    $0x28,%esp
  int ret = join(&stack);
 340:	50                   	push   %eax
 341:	e8 8a 01 00 00       	call   4d0 <join>
 346:	ba 80 0c 00 00       	mov    $0xc80,%edx
 ...
 355:	eb 14                	jmp    36b <thread_join+0x3b>
 357:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
 35e:	66 90                	xchg   %ax,%ax
  for(int i = 0; i < MAX_PROC; i++){
 360:	83 c1 01             	add    $0x1,%ecx
 363:	83 c2 0c             	add    $0xc,%edx
 366:	83 f9 40             	cmp    $0x40,%ecx
 369:	74 3f                	je     3aa <thread_join+0x7a>
    if(ptrs[i].busy == 1 && ptrs[i].stack == stack){
 36b:	83 3a 01             	cmpl   $0x1,(%edx)
 36e:	75 f0                	jne    360 <thread_join+0x30>
 370:	39 42 08             	cmp    %eax,0x8(%edx)
 373:	75 eb                	jne    360 <thread_join+0x30>
      free(ptrs[i].ptr);
 375:	8d 34 49             	lea    (%ecx,%ecx,2),%esi
 378:	83 ec 0c             	sub    $0xc,%esp
 37b:	c1 e6 02             	shl    $0x2,%esi
 37e:	ff b6 84 0c 00 00    	pushl  0xc84(%esi)
 384:	e8 d7 03 00 00       	call   760 <free>
      ptrs[i].stack = NULL;
      ptrs[i].busy = 0;
      ptrs[i].ptr = NULL;
      break;
 389:	83 c4 10             	add    $0x10,%esp
      ptrs[i].stack = NULL;
 38c:	c7 86 88 0c 00 00 00 	movl   $0x0,0xc88(%esi)
 393:	00 00 00 
      ptrs[i].busy = 0;
 396:	c7 86 80 0c 00 00 00 	movl   $0x0,0xc80(%esi)
 39d:	00 00 00 
      ptrs[i].ptr = NULL;
 3a0:	c7 86 84 0c 00 00 00 	movl   $0x0,0xc84(%esi)
 3a7:	00 00 00 
    }
  }
  return ret;  
...
void lock_init(lock_t *lock)
{
 ...
 3ca:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  lock->turn = 0;
 3d0:	c7 40 04 00 00 00 00 	movl   $0x0,0x4(%eax)
}
 3d7:	5d                   	pop    %ebp
 3d8:	c3                   	ret    
...
void lock_acquire(lock_t *lock){
 ...
 3ec:	53                   	push   %ebx
 3ed:	8b 55 08             	mov    0x8(%ebp),%edx
    __asm__ volatile
 3f0:	f0 0f c1 0a          	lock xadd %ecx,(%edx)
 3f4:	31 db                	xor    %ebx,%ebx
 3f6:	83 c2 04             	add    $0x4,%edx
 ...
 408:	75 f6                	jne    400 <lock_acquire+0x20>
}
 40a:	5b                   	pop    %ebx
 40b:	5d                   	pop    %ebp
 40c:	c3                   	ret    
...
void lock_release(lock_t *lock){
 ...
 41a:	83 40 04 01          	addl   $0x1,0x4(%eax)
}
 41e:	5d                   	pop    %ebp
 41f:	c3                   	ret    
 ...
 4ec:	89 d1                	mov    %edx,%ecx
{
 ...
 519:	0f b6 92 00 09 00 00 	movzbl 0x900(%edx),%edx
 ...
 55e:	56                   	push   %esi
 55f:	57                   	push   %edi
 560:	e8 e3 fe ff ff       	call   448 <write>
  while(--i >= 0)
...
printf(int fd, const char *fmt, ...)
{
 ...
 639:	e8 0a fe ff ff       	call   448 <write>
        putc(fd, c);
      } else {
 ...
 64d:	e8 f6 fd ff ff       	call   448 <write>
  for(i = 0; fmt[i]; i++){
 ...
 6c1:	88 45 e7             	mov    %al,-0x19(%ebp)
  write(fd, &c, 1);
 6c4:	e8 7f fd ff ff       	call   448 <write>
        ap++;
 ...
 714:	eb 1a                	jmp    730 <printf+0x1a0>
          s = "(null)";
 716:	bb f8 08 00 00       	mov    $0x8f8,%ebx
        while(*s != 0){
 ...
 73b:	57                   	push   %edi
 73c:	53                   	push   %ebx
 73d:	e8 06 fd ff ff       	call   448 <write>
        while(*s != 0){
...
malloc(uint nbytes)
{
 ...
 85b:	e8 50 fc ff ff       	call   4b0 <sbrk>
  if(p == (char*)-1)

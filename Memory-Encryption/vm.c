...
deallocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
  ...
  cprintf("deallocuvm: oldsz: %p, newsz: %p, a: %p\n", (char *) oldsz, (char *)
newsz, (char *)a);
...
uva2ka(pde_t *pgdir, char *uva)
{
  ...
  if(((*pte & PTE_P) | (*pte & PTE_E)) == 0) {
	cprintf("uva2ka DEBUG:\n");
	cprintf("PTE_P: %d, PTE_E: %d\n", (*pte & PTE_P), (*pte & PTE_E)) ;
    return 0;
	}
  if((*pte & PTE_U) == 0)
    return 0;
...
int mencrypt(char *virtual_addr, int len) {
  ...
  for (int i = 0; i < len; i++) {
    ...
    // P4 test 3 fix: when encrypt is called, clear PTE_A bit
    *mypte &= ~PTE_A;
  }
...
int getpgtable(struct pt_entry* pt_entries, int num, int wsetOnly) {
  ...
  for (;;uva -=PGSIZE)
  {
    ...
    // If PTE_E is set, it means the address is not in ws clock queue
...
pte_t *walkpgdir_public(pde_t *pgdir, char *va)
{

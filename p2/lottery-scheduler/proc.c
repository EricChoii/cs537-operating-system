...
fork(void)
{
  ...
  *np->tf = *curproc->tf;
  ...
  // Chlid will have the same number of tickets as its parent
...
struct proc *hold_lottery(int total_tickets) {
  ...
  struct proc* p;
  int ticket_count = 0;
  uint winner_ticket_number = rand() % total_tickets + 1;
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
    if(p->state != RUNNABLE)
      continue;
...
// Return total number of tickets for runnable processes
...
scheduler(void)
{
  ...
  for(;;){
    ...
    // Boost Managemenet
    for(struct proc* bp = ptable.proc; bp < &ptable.proc[NPROC]; bp++) {     
      // Increase boostsleft for blocked process
      if(bp->state == SLEEPING && bp->blocked)
        bp->boostsleft++;
      if(bp->state != RUNNABLE)
        continue;      
      // Decrease boostsleft for every RUNNABLE process
      if(bp->boostsleft > 0)
        bp->boostsleft--;
    }
...
wakeup1(void *chan)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
    if(p->state == SLEEPING && p->chan == chan) {      
      // boost for sleep syscall
      if(p->sleep > 0) {
        p->sleep--;
        p->boostsleft++;
        continue;
      }
      // if process is blocked, then skip
      if(p->blocked)
        continue;
      p->state = RUNNABLE;
    }
...
// return  basic information about each running process
int
